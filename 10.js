...
function BotProcessor(alarmedValues) {
	...
	function giveValue(botNumber, value) {
		...
		if (bot.values.length == 2) {
			...
			if (bot.lowTo >= 0) {
				giveValue(bot.lowTo, values[0]);
			} else {
				putToOutput(bot.lowTo, values[0]);
			}

			if (bot.highTo >= 0) {
				giveValue(bot.highTo, values[1]);
			} else {
				putToOutput(bot.highTo, values[1]);
			}
		}
	}

	function putToOutput(outputNum, value) {
		outputNum = -outputNum - 1;
		
		if (output[outputNum] == undefined) {
			output[outputNum] = [];
		}

		output[outputNum].push(value);
	}
